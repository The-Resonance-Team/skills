...
- **Vendored skill** — a skill installed by `scripts/install.mjs` via `npx skills@latest add <github-url> -g` (mattpocock, ponytail, context-mode): global scope, symlinked by default, agent choice interactive. Never edited in place — edit upstream or re-install.
