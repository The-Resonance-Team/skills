...
- **Vendored skill** — a skill installed into a client from an upstream GitHub repo by that client's loader (e.g. `npx skills add <github-url> -g`, `npx firecrawl init`). Never edited in place — edit upstream or re-install.
- **Team MCP stack** — the five servers defined in `mcp.json`: context7, chrome-devtools, context-mode (stdio) and github, firecrawl (streamable-http). Consumers copy the servers into their client configs (opencode `mcp`, Claude Code `mcpServers`). Secret-bearing fields are `${VAR}` placeholders translated per client env rules; real keys are never committed.
