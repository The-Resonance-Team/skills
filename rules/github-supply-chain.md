...
### 3. Isolate `pnpm/setup` per job on shared-HOME runners

`pnpm/setup` self-installs to `~/setup-pnpm` by default. Self-hosted runners share `$HOME` across parallel jobs, and concurrent self-installers race linking bins (`ENOENT ... chmod '.../pnpm/pnpm'`, exit 1 — incident 2026-09: lint + typecheck failed while test passed on the same push). Point `dest` at the per-job temp dir. Drop when back on github-hosted runners.
...
| CI      | `pnpm/setup@v2` with no version input (reads the same field)                                           |
| Docker  | derive at build time — rules/general.md §24                                                            |

`pnpm/action-setup` stays for pnpm 10 and older only; 11+ uses `pnpm/setup`.
