...
4. **Global `whitelist: true, forbidNonWhitelisted: true` 422s on any field the DTO doesn't declare — including a field a caller has sent for months.** A query/body param not present on the DTO is rejected outright, not silently stripped. This bites hardest when one side changes without the other: a frontend still sending a param the DTO never had (renamed, dropped, or never added), or a DTO trimmed without checking every caller. The call 422s in every environment, permanently — and if the caller swallows the rejected response, the visible symptom is a silently blank UI section, not an error toast, which makes it look like a data problem instead of a contract mismatch. Before adding a param on the client, confirm the DTO declares it; before removing a DTO field, grep every caller for the param name (precedent: XaDaoXa 2026-08, three independent instances of this exact bug in one session — a report-window param, an audit-log filter set, and a citizens-summary omission).
5. **A `@IsDateString()` field with no `@Type(() => Date)` transform can 500 at the database instead of 422 at the pipe.** `@IsDateString() dueAt: string` validates both `"2026-08-15T10:00:00Z"` and the bare date `"2026-08-15"` — but a Prisma `DateTime` column rejects the date-only form with a raw driver error ("premature end of input. Expected ISO-8601 DateTime"), surfacing as an unhandled 500 on exactly the shape the DTO's own doc-comment example may show, and reachable from any plain `<input type="date">` on the frontend, not just a malformed client. Use `@Type(() => Date) @IsDate()` for any field written into a `DateTime` column — it coerces both forms to a real `Date` before Prisma ever sees a string. A repo-mocking unit test does not catch this; only a real-database write does (precedent: XaDaoXa 2026-08, found twice — once in a controller shipped as `@IsDateString()`, once as a latent, not-yet-triggered instance of the same pattern found by inspection after the first).

## Routing (mandatory)

25. **A literal-path route registered after a `:param` route on the same base is permanently unreachable.** `GET /things/:id` matches before `GET /things/summary` ever gets a chance if `:id` is declared first — the literal segment "summary" is swallowed as an `:id` value. This shadowing happens at two levels: decorator order inside one controller, and `@Module` import order across two controllers that share a `@Controller('prefix')` base (whichever module is imported first wins the route table for any overlapping sub-path). Always register literal-path handlers before `:param` handlers in the same controller; when two controllers share a base path, audit with `grep -hoE "@Controller\('[^']+'\)" src -r | sort | uniq -d` and confirm their sub-resource paths don't overlap, or that the literal one is registered first (precedent: XaDaoXa 2026-08 — found once as a within-controller bug, then swept for and found again as a cross-module instance of the identical pattern).

## Swagger docs (mandatory)

22. **No `@ApiProperty` when `classValidatorShim` is enabled** — `nest-cli.json` enables `@nestjs/swagger` plugin with `classValidatorShim: true` + `introspectComments: true`. The plugin already infers `type/required/enum` from `class-validator` decorators, so DTOs must not duplicate `@ApiProperty`/`@ApiPropertyOptional`. Keep only `class-validator` (`@IsString()`, `@IsOptional()`, `@IsEnum()`, `@Min()`, etc.) + `class-transformer` (`@Type()`, `@Transform()`). If Swagger needs an `example` or `description`, add JSDoc `/** example: Sân Pickleball Tao Đàn */` above the field — the plugin's `introspectComments` will pick it up. Do not re-add `@ApiProperty` for docs-only purposes; it is the over-engineering cut from ponytail audit 2026-08 (634 decorators removed). Revisit if `classValidatorShim` is ever disabled — then `example`/`description` must return as `@ApiProperty`.
