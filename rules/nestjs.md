...
19. **Repo files live in `module_name/repo/` — no `repositories/prisma/prisma-` nesting** — The repo layer per module is one flat directory: `modules/<m>/repo/<name>.repo.ts` for the Prisma-backed class, `<name>.repository.interface.ts` for its interface, and `repo/index.ts` as the barrel (class + interface + row types re-exported; consumers import from the barrel, never the deep file path). Do NOT create `repositories/prisma/prisma-x.repo.ts` + `repositories/interfaces/` — that nesting and `prisma-` prefix are banned (migrated 2026-08, TaxEasy `refactor/api-architecture`: 298 files). Class names keep the `Prisma` prefix (`PrismaPaymentIntentRepository` in `repo/payment-intent.repo.ts`); interfaces keep `I` prefix + `.repository.interface.ts` suffix. Cross-module consumers import the class via `@/modules/<m>/repo`. Shared base: `common/repo/base-prisma.repo.ts`. Files at `repo/` depth import siblings with `./name.repo`, the barrel with `'.'`.
